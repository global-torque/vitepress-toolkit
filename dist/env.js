...
export function readEnvFile(filePath) {
    ...
    return fs.readFileSync(filePath, 'utf8')
        ...
        const quote = value[0];
        accumulator[key] = ((quote === '"' || quote === "'") && value.endsWith(quote)
            ? value.slice(1, -1)
            : value);
        return accumulator;
    }, {});
}
export function loadVitePressEnv({ cwd = process.cwd(), mode = '', localFallbackPath, includeGitHash = false, includeBuildTimestamp = false, } = {}) {
    if (includeGitHash) {
