...
const normalizeHeadValue = (value: unknown) => {
...
export const sanitizeHeadEntries = (head: unknown[]): HeadEntry[] => (
  head
    .filter((entry): entry is [string, Record<string, unknown>?, unknown?] => (
      Array.isArray(entry)
      && typeof entry[0] === 'string'
    ))
    .map(([tag, attrs = {}, innerHTML]) => {
      const attrsSafe: HeadAttrs = {};
...
const toAbsoluteUrl = (url: string | undefined, frontendUrl: string | undefined) => {
  if (!url) {
    return undefined;
  }
...
const getFrontmatterImage = (frontmatter: PageData['frontmatter']) => frontmatter.image;
...
const resolveRobotIndex = (value: SeoEnv['robotIndex']) => {
...
export function createTransformPageData({
  env,
  normalizeFrontmatter,
  stripHtmlAndMarkdown,
  generateJsonLd,
  beforeNormalize,
  resolveDescription = (pageData, seoEnv) => {
  ...
  defaultImage = '/images/sharing.png',
  includeMetaTitle = false,
  includeTwitterDescription = true,
  includeSiteName = false,
  jsonLdMode = 'when-present',
  ogAttribute = 'name',
  sanitizeHead = false,
}: CreateTransformPageDataOptions) {
